...
from app.schemas.shipments import CreateShipmentSchema, UpdateShipmentSchema, GetShipmentSchema
...
def create_shipment(shipment: CreateShipmentSchema, db: Session = Depends(get_db)):
    instance = Shipment.create_from_schema(
        obj_in=shipment,
        session=db,
    )
...
@router.get("/details", summary="Get a specific shipment details", response_model=GetShipmentSchema)
